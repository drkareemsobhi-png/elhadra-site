(function (global) {
  ...
  var PENDING_KEY = 'elhadra_pending_order_v1';
  ...
  function createClientOrderId() {
    ...
    return 'ehw_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 18);
  }
  ...
  function checkReceiptOnce(webhook, clientOrderId) {
    return new Promise(function (resolve) {
      var callback = 'ehReceipt_' + Math.random().toString(36).slice(2, 18);
  ...
  function renderSuccess(container, options, receipt) {
    ...
    // الإجمالي اللي بيتعرض هو اللي السيرفر حسبه وسجّله — رقم المتصفح احتياطي بس.
    var total = receipt && receipt.total != null && receipt.total !== '' ? receipt.total : options.total;
    container.innerHTML =
      '<div style="text-align:center;padding:34px 10px 20px">' +
